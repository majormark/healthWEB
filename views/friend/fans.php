...
<?php 
    			         ...
    			         $sql =<<<EOF
      SELECT * FROM followership fs, user u where fs.fans=u.nickname and followed='$user';
EOF;
    			         $ret = $db->query($sql);
    			         if(!$ret){
    			             echo $db->lastErrorMsg();
    			         ...
    			         while($row=$ret->fetchArray(SQLITE3_ASSOC)){
    			             $fans_list.=formatSay($row['fans'],$row['address']);
    			             $count+=1;
    			         }
    			        
    			         function formatSay($fans,$address){
    			             $str= '
    			               <div class="friend-list"> 
                <a href="#"><img src="http://localhost/healthWEB/static/vendor/image/2.jpg" alt="" /></a> 
                <div class="friend-info">
						<span class="left"><a href="#"><strong>'.$fans.'</strong></a></span> <span
							class="right">';
							if(isJoin($fans,$_SESSION['username'])){
    			                 $str.='<a href="http://localhost/healthWEB/controller/friend/follow_controller.php?followed='.$fans.'&op=out&source=fans">取消关注</a></span><br />';
							}else{
							    $str.='<a href="http://localhost/healthWEB/controller/friend/follow_controller.php?followed='.$fans.'&op=join&source=fans">关注</a></span><br />';
							}
		    ...
		    ';
    			     ...
    			     function isJoin($fans,$followed){
    			         $db=new MyDB();
    			         if(!$db){
    			             echo $db->lastErrorMsg();
    			         }
    			         $sql =<<<EOF
      SELECT * FROM followership where fans='$followed' and followed='$fans' 
EOF;
    			         $ret = $db->query($sql);
